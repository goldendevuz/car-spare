...
async def nav_search(message: Message, state: FSMContext, api):
    ...
    if city_id:
        ...
        await message.answer("Zapchast nomini yozing (masalan: Cobalt old fara):", reply_markup=main_menu())
        return
...
async def nav_city(message: Message, state: FSMContext, api):
    ...
    current_city_id = get_last_city(message.from_user.id)
...
async def set_city(cb: CallbackQuery):
    ...
    await cb.message.edit_text(f"✅ Hudud tanlandi: {city_name or city_id}")
    await cb.message.answer("Bosh menyu", reply_markup=main_menu())
    await cb.answer()